...
class IntegrateCurrent(object): # pylint: disable=useless-object-inheritance
    ...
    def integrate(self):
        ...
        if target_number < integral:
            ...
            msg = 'Total charge deposited: {0} pmol.\n'.format(to_pmol(integral))
            msg += 'Present coverage: {0} %\n'.format(present_coverage)
            msg += 'Time until {0} % coverage: {1} hr {2} min {3} sec'
        ...
        if present_coverage is None:
            print('Total charge deposited: {0} pmol.\n'.format(to_pmol(integral)))
    ...
    def get_std(self, averaged_gradient):
        ...
        while True:
            ...
            if abs(averaged_gradient[offset+counter]) > std_gradient:
                print('Gradient computed up to index {0}'.format(offset+counter))
                break
        return std_gradient
